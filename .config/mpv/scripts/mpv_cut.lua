...
function get_incremental_filename(directory, original_filename, extension)
    -- Log the input parameters
    msg.info(string.format("get_incremental_filename called with directory: %s, original_filename: %s, extension: %s", directory, original_filename, extension))
    
    -- Ensure the directory does not end with a slash
    if directory:sub(-1) == "/" or directory:sub(-1) == "\\" then
        directory = directory:sub(1, -2)
    end
    msg.info(string.format("Cleaned directory: %s", directory))

    -- Remove the extension from the original filename
    local base_name = original_filename:gsub("%." .. extension .. "$", "")
    msg.info(string.format("Base name: %s", base_name))

    local index = 1
    local new_filename
    repeat
        -- Create the new filename with an incremented index
        new_filename = string.format("%s\\COPY_%d_%s.%s", directory, index, base_name, extension)
        msg.info(string.format("Trying new filename: %s", new_filename))
        index = index + 1
    until not utils.file_info(new_filename)

    -- Log the final new filename
    msg.info(string.format("Final new filename: %s", new_filename))
...
function mark_pos(is_web)
    ...
    msg.info(string.format("Current position: %s", current_pos))
    ...
    log(msg.info, string.format("Cut duration: %s", vars.pos.cut_duration))

    -- Generate the output file name
    local output_name = get_incremental_filename(vars.directory, vars.only_filename, settings.video_extension)
    log(msg.info, string.format("Output name: %s", output_name))
    ...
    -- Log the success of the cut
    log(msg.info, string.format("Cut successful, output file: %s", output_name))

    -- Resize video
    if is_web then
        local output_name_resized = get_incremental_filename(vars.directory, vars.only_filename, settings.video_extension)
        log(msg.info, string.format("Output name resized: %s", output_name_resized))
        ...
        if not status then
            ...
            log(msg.info, string.format("Deleted temporary file: %s", output_name))
        end
        ...
        if not status then
            ...
            log(msg.info, "Deleted ffmpeg2pass-0.log")
        end
        ...
        if not status then
            ...
            log(msg.info, "Deleted ffmpeg2pass-0.log.mbtree")
        end
...
mp.register_event("file-loaded", function()
    ...
    -- Populate variables
    vars.path, vars.filename, vars.only_filename, vars.directory = path, filename, only_filename, directory
end)
